...
enum Interval extends AbsDomain:
  case Top
...
given IntervalExp: Abs[Interval, Expr, Val] with
  ...
  override def minus(left: Interval, right: Interval): Interval =
    import Interval.*
    (left, right) match
      case (Top, _) | (_, Top) | (GreatestLower(_), LeastUpper(_)) |
          (LeastUpper(_), GreatestLower(_)) =>
        Top
      ...
      case (UpperLower(a1, b1), UpperLower(a2, b2)) =>
        UpperLower(a1 - a2, b1 - b2)
