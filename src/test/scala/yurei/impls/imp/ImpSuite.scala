...
class ImpSuite extends munit.FunSuite {
  ...
  // [2,2] - [2,2] => [0,0]
  test("2 - 2 ") {
    val expr = Minus(Expr.V(Val.Num(2)), Expr.V(Val.Num(2)))
    assertEquals(step_forward(expr, List(SignExpr, IntervalExp)), List())
  }
}
...
//    assertEquals(step_forward(expr, List(IntervalExp)),List(expected))
//  }
//
//  test("1 - 1 -> [0,0]") {
